...
def main():
    ...
    spec = importlib.util.spec_from_file_location('candidate', '/Users/twishamehta/tiktok/kuairand-starter-kit/runs/kairos_submission_repro/backtest_confirm/candidate.py')
    m = importlib.util.module_from_spec(spec); spec.loader.exec_module(m)
    from kairos.agent.context import make_context
    ctx = make_context('backtest_a')
    t0 = time.time()
    built = m.build(ctx)
    cfg = {}
    if len(built) == 3:
    ...
    np.save('/Users/twishamehta/tiktok/kuairand-starter-kit/runs/kairos_submission_repro/backtest_confirm/X.npy', X)
    json.dump({'names': list(names), 'seconds': round(time.time()-t0,1),
               'train_cfg': dict(cfg or {})}, open('/Users/twishamehta/tiktok/kuairand-starter-kit/runs/kairos_submission_repro/backtest_confirm/meta.json', 'w'))
    print("OK")
try:
    main()
except Exception:
